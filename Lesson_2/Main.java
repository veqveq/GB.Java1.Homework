...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Задание 7");
                ...
                /*1 Вариант решения
        Элементы массива просто сдвигаются на указанное число позиций, пустые позиции заполняются нулями
         */
        ...
        /*2 Вариант решения
        Массив становится замкнутым.
        При сдвижке вправо последние элементы перемещаются в начало, при сдвижке влево первые элементы уходят в конец
         */
        int [] NotMovedArrRound = {1, 13, 25, 12, 6, 20, -2, 15, -34, -55};     //Инициализация входного массива
        offset = 5;                                                             //Задание числа-количества сдвигаемых позиций
        RoundMoveArr(NotMovedArrRound, offset);                                 //Вызов метода круговой сдвижки
    }
    ...
    public static void FindAxisOfSymmetry (int symmetricArr[]){                 //Объявление метода. Входные данные - массив symmetricArr
       ...
       Задание 7.
    Вариант 1.
    Метод обычной сдвижки элементов в массиве
       */
    public static void MoveArr(int NotMovedArr[], int offset){      //Объявление метода. Входные данные - массив NotMovedArr, число сдвигаемых позиций offset
     ...
     /*
        Задание 7.
        Вариант 2.
        Метод круговой сдвижки элементов в массиве
        */
    public static void RoundMoveArr(int NotMovedArrRound[], int offset) {   //Метод для круговой сдвижки элементов. 2 Вариант задачи
        if (Math.abs(offset) > NotMovedArrRound.length) {                   //Если количество сдвижек больше чем длина массива
            offset = offset % NotMovedArrRound.length;                      //То сократить количество сдвижек через деление по модулю, выкинув все полные циклы прокрутки
        }                                                                   //В результате должно остаться число, меньшее по модулю чем длина массива
        if (offset < 0) {                                                   //Если сдвижка отрицательная и массив нужно двигать влево
            offset = NotMovedArrRound.length + offset;                      //То заменяем число сдвижек на эквивалентное положительное и двигаем массив вправо
        }

        for (int i = 0; i < offset; i++) {                                  //Цикл отвечающий за количество сдижек элементов
            int lastElement = NotMovedArrRound[NotMovedArrRound.length - 1];//Перед выполнением непосредственной сдвижки кладем в переменную последний элемент
            for (int j = NotMovedArrRound.length - 1; j > 0; j--) {         //Цикл выполняющий сдвижку элементов на 1 вправо. Прерывается, когда доходит до первого элемента
                NotMovedArrRound[j] = NotMovedArrRound[j - 1];              //Каждый следующий элемент заменяется предыдущим
            }
            NotMovedArrRound[0] = lastElement;                              //После отработки вложенного цикла 1 элемент заменяется последним из переменной
        }
            System.out.println(Arrays.toString(NotMovedArrRound));          //Вывод сдвинутого массива
    }
}
